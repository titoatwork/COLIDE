"""
COLIDE – ToN-IoT Distillation v2 (Aggressive SMOTE, same as BoT‑IoT success)
Applies massive SMOTE to weak classes, keeps majority classes large.
Distills from RF teacher (200 trees) with α=0.5.
...
# ============================================================
# Data Loading (Aggressive SMOTE)
# ============================================================
def load_data_toniot():
    """Load ToN-IoT, apply aggressive SMOTE to weak classes."""
    ...
    # Define SMOTE targets (based on BoT‑IoT success pattern)
    target_map = {
        'backdoor': 50000,
        'ddos': 50000,
        'dos': 50000,
    ...
    for name, target in target_map.items():
        ...
        current_count = Counter(y_train_raw)[class_idx]
        if current_count < target:
            smote_strategy[class_idx] = target
    
    print(f"Applying SMOTE to {len(smote_strategy)} classes...")
    smote = SMOTE(sampling_strategy=smote_strategy, random_state=SEED, k_neighbors=min(3, min(Counter(y_train_raw).values())-1))
    ...
    print(f"Train (after SMOTE): {X_train_smote.shape[0]:,} | Val: {X_val.shape[0]:,} | Test: {X_test.shape[0]:,}")
...
def main():
    ...
    print("="*60 + "\nCOLIDE DISTILLATION v2 (Aggressive SMOTE) - ToN-IoT\n" + "="*60)
    ...
    best_f1 = 0.0
    patience_counter = 0
    save_path = "model/best_model_toniot_distill_v2.pth"
    ...
    results = {
        'dataset': 'toniot',
        'method': 'Aggressive SMOTE + Distillation',
        'alpha_kd': args.alpha,
    ...
    with open("benchmarks/results/distill_toniot_v2.json", 'w') as f:
