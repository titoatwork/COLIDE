#!/usr/bin/env python3
"""
Post-run helper for WP5a ablation ladder.
...
def main() -> int:
    ...
    p.add_argument("--tag", default="ablation_ladder")
    args = p.parse_args()
    ...
    print("\n=== MANIFEST ROWS (JSON) ===")
    for path in sorted(out_dir.glob("A*_seed*.json")):
        print(
            f"| `{path.relative_to(PROJECT_ROOT)}` | yes | `{md5(path)}` | "
            f"{path.stat().st_size} |"
        )
    print(f"| `{summary_path.relative_to(PROJECT_ROOT)}` | yes | `{md5(summary_path)}` | "
          f"{summary_path.stat().st_size} |")
    print("\n=== CHECKPOINTS ===")
    for path in sorted(ckpt_dir.glob("A*_seed*.pth")):
        print(
            f"| `{path.relative_to(PROJECT_ROOT)}` | yes | `{md5(path)}` | "
            f"{path.stat().st_size} |"
        )
