"""
COLIDE – BoT-IoT Distillation Training (with temperature scaling)
Supports alpha sweep, temperature scaling, custom suffix,
Focal Loss, and SMOTE scale for majority classes.
"""
...
class FocalLoss(nn.Module):
    ...
    def __init__(self, alpha=None, gamma=2.0, reduction='mean'):
        super().__init__()
        self.alpha = alpha          # per-class weights tensor (optional)
        self.gamma = gamma
...
def load_data_botiot(smote_scale=1.0):
    """Load BoT-IoT with SMOTE (original targets), optionally scaled."""
    ...
    # SMOTE targets (same as original)
    strat = {}
    ...
    # SMOTE scaling for majority classes (NEW)
    if smote_scale != 1.0:
        for i, name in enumerate(class_names):
            if name in ['DDoS', 'DoS']:
    ...
    print(f"Dataset: BoT-IoT | Classes: {num_classes}")
...
def main():
    ...
    print("="*60)
    print(f"COLIDE DISTILLATION — BoT-IoT | α={args.alpha} T={args.temperature}")
    print("="*60)

    # Pass smote_scale to data loader
    X_train, y_train, X_val, y_val, X_test, y_test, class_names, num_classes = \
        load_data_botiot(smote_scale=args.smote_scale)
    ...
    if args.focal_gamma > 0.0:
        ce_loss = FocalLoss(alpha=None, gamma=args.focal_gamma)
    ...
    print(f"\n{'='*60}")
    print(f"TRAINING (α={args.alpha}, T={args.temperature})")
    print(f"{'='*60}")

    best_f1 = 0.0
    patience_counter = 0
    suffix_str = f"_{args.suffix}" if args.suffix else ""
    save_path = f"model/best_model_botiot_distill{suffix_str}.pth"
    ...
    results = {
        ...
        'smote_scale': args.smote_scale,
        'macro_f1': float(macro_f1),
        'weighted_f1': float(weighted_f1),
        'accuracy': float(acc),
        'best_val_f1': float(best_f1)
    }
    results_path = f"benchmarks/results/distill_botiot{suffix_str}.json"
