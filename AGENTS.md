...
`fused_pipeline.cu` skips Block 3. Collect full-model PyTorch latency for absolute numbers; publish
**per-block** ratios only (Block 3 is the preferred head-to-head). Update README/paper claims only
from accepted cross-day compare artifacts.
...
`fused_pipeline.cu` is **not** a full V3 forward: it measures Blocks 1+2+4 chained and does **not**
feed a real Block-3 BiLSTM output into Block 4 (additive reconstruction only). V3 PyTorch also runs
attention + LayerNorm + global average pooling after BiLSTM — CUDA implements none of those. Do not
publish a full-pipeline CUDA/PyTorch speedup until that parity gap is fixed; per-block (esp. Block 3)
comparisons remain valid. Weights are exported from the trained PyTorch model via
