...
|----|------|-------|---------|-------|--------|
| WP0 | UM DICC Day1+Day2+compare+scp | 0 | — | User + agent | **DONE** — 6 SUCCESS on laptop+git; V100 compares + A100 allow-dirty compares. See `PRE_MANUSCRIPT_CLOSURE.md` |
| WP0b | Extract DICC JSON tables + fork decision | 0 | WP0 | Agent+user | **DONE** — `DICC_EXTRACTION_TABLES.md`; fork: B3 CUDA loses to PT on servers |
...
| WP6c | Re-DICC if model changed | 6 | WP6a | User/ops | **N/A** — champion unchanged; **full multi-compiler + Option A already measured 2026-08-12** |
| WP6d | DICC multi-compiler matrix | 6 | WP0 | Agent | **DONE 2026-08-12** eager/compile/ORT/TRT V100S+A100 (`DICC_MULTI_COMPILER_MATRIX.md`) |
...
| WP9c | Camera-ready draft PDF + remaining figures | 9 | WP9b | User+agent | **DONE 2026-07-22** draft + PI polish; **DICC cells ready to insert** from claim map (writing next) |
| WP9d | Insert DICC Option A + multi-compiler into manuscript | 9 | WP0/WP6d | User+agent | **NEXT** — use `CLAIM_MAP_PREWRITE.md` only |

---

## What to run **right now** (next chat)

| Priority | Action |
|----------|--------|
| 0 | **Manuscript insert** of DICC tables from `CLAIM_MAP_PREWRITE.md` (no invent) |
| 1 | Optional: PI fills authors / venue class file / BibTeX |
| 2 | Optional science: S1b clean A100 / Nsight / B3 optim only if PI prioritizes |
| 3 | Keep claims green after prose/number edits; rebuild PDF via `scripts/build_manuscript_pdf.py` |
